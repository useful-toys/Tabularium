---
name: spec-check
description: Verifica a saúde da spec viva - roda o check determinístico (scripts/spec.mjs) e faz uma revisão semântica de drift entre a spec (product.md, model.md e documentos técnicos) e o comportamento do código. Use antes de abrir um PR, ao revisar um PR, ou quando suspeitar que a spec não reflete o código.
---
...
- **Código sem item**: comportamento observável relevante que o `product.md` não descreve, ou estrutura do domínio que o `model.md` não descreve.
- **Modelo**: entidade, relação, estado ou invariante do `model.md` que o código não sustenta.
- **Divergência**: o código faz diferente do que o item `✓` afirma.
- **Forma**: implementação, tela ou navegação no `product.md`; detalhe de banco no `model.md`; conceito repetido em duas seções; justificativa técnica.
...
- tipo;
- local (`product.md` ou `model.md` › seção › item);
