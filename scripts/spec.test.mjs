...
test('histórico aceita entrada de organização', () => {
...
test('criar, editar e desistir de ⇢ não exige código', () => {
  const created = '- ✓ A\n  - ✓ B ⇢ B2\n- C\n';
  const edited = '- ✓ A\n  - ✓ B ⇢ B3\n- C\n';
  const empty = { resolutions: [], rewrites: [], marks: [] };
  assert.deepEqual(classifyProductDiff(base, created), empty);
  assert.deepEqual(classifyProductDiff(created, edited), empty);
  assert.deepEqual(classifyProductDiff(created, base), empty);
});

test('mover linhas ✓ não exige código', () => {
  assert.deepEqual(classifyProductDiff(base, '- C\n- ✓ A\n  - ✓ B\n'), { resolutions: [], rewrites: [], marks: [] });
});

test('resolver ⇢ é entrega', () => {
...
function repo() {
  ...
  return { dir, commit, edit, cleanup: () => rmSync(dir, { recursive: true, force: true }) };
}
...
test('check: entrega de ⇢ sem código falha, com código passa', () => {
  const r = repo();
  try {
    r.edit('- ✓ Desfazer contagens: até as 3 últimas', '- ✓ Desfazer contagens: até as 3 últimas ⇢ Desfazer contagens: até as 5 últimas');
    r.commit('compromisso');
    assert.deepEqual(check(r.dir, { base: 'HEAD~1' }).errors, []);

    r.edit('- ✓ Desfazer contagens: até as 3 últimas ⇢ Desfazer contagens: até as 5 últimas', '- ✓ Desfazer contagens: até as 5 últimas');
    r.commit('entrega sem código');
    const errors = check(r.dir, { base: 'HEAD~1', labels: ['spec-only'] }).errors;
